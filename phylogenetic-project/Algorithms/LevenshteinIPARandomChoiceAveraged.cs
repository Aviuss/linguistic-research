...
public class LevenshteinIPARandomChoiceAveraged
{
        ...
        (List<string[]> inputText1, List<string[]> inputText2, long randomSize = 10000)
    {
        ...
        bool isBruteForce = amountOfTotalCombinations <= randomSize;
        if (isBruteForce)
        {
        ...
        foreach (var comboMem in cartesian)
        {
            ...
            string txt1String = txt1.ToString();
            string txt2String = txt2.ToString();
        ...
        return new LevenshteinIndividualDataDecimal(
            (decimal)avgLevenshteinDistance / (isBruteForce ? amountOfTotalCombinations : randomSize),
            (decimal)avgMaxDistance / (isBruteForce ? amountOfTotalCombinations : randomSize)
        );
    }

    public static IEnumerable<ReadOnlyMemory<T>> CartesianProductSpan<T>(IReadOnlyList<IReadOnlyList<T>> sequences)
    {
    ...
    public static IEnumerable<ReadOnlyMemory<T>> CartesianProductSpanRandomChoice<T>(IReadOnlyList<IReadOnlyList<T>> sequences, long iterations)
    {
