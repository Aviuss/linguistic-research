...
public class IPA
{
    ...
    public static SortedSet<string> ConvertToIpa_ReturnLettersWhichDontConvert(string text, Persistance.LanguageRules rule)
    {
        text = text.ToLowerInvariant();
        SortedSet<string> lettersThatDontConvert = new();
        int maxCharLen = 0;
        foreach (var element in rule.Rules)
        {
            maxCharLen = Math.Max(maxCharLen, element.Key.Length);
        ...
        while (index < text.Length)
        {
            ...
            if (addChar == null)
            {
                lettersThatDontConvert.Add(charList[0]);
                index += 1;
            }
